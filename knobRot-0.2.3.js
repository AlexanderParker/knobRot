	...
	var methods = {
				...
				if ($this.is('input:text')) {					
					...
					$(document).on('mousemove.knobRot', function( event ) {
						...
						if ( $('body').data('knobRot').dragging == true ) {
							...
							// Redraw
							methods.updateCallback($('body').data('knobRot').knobDiv.data('knobRot').realValue);
						}																
					});
					...
					knobDiv.find('*').on('change', function() {
		...
		stopCallback: function() {
			if (typeof console !== 'undefined' && typeof(console.log !== 'undefined')) {
				console.log('stopCallback has been deprecated in favour of event listeners.  You should not have to worry about it anymore.');
			}
		}
	};	
	/**
